...
class Picker extends Component {
  ...
  state = {
    showType: this.getDefaultShowType(),
    activeDate: this.getActiveDate()
  }

  componentWillReceiveProps(nextProps) {
    this.setState({showType: this.getDefaultShowType(nextProps), activeDate: this.getActiveDate(nextProps)});
  }
  ...
  componentDidUpdate() {
    ...
    if (!visible) {
      this.refs.pickerContent.style.top = `${TOP_VALUE_TO_HIDE}px`;
      return;
    }
    const {body} = window.document;
    const targetRect = target.getBoundingClientRect();
    const bodyRect = body.getBoundingClientRect();
    const pickerRect = this.refs.pickerContent.getBoundingClientRect();
    if (!position) {
    ...
    if (position.indexOf('bottom') === 0) {
      this.refs.pickerContent.style.top = targetRect.top - bodyRect.top + targetRect.height + 'px';
    } else {
      this.refs.pickerContent.style.top = targetRect.top - bodyRect.top - pickerRect.height + 'px';
    }
    if (position.indexOf('left') >= 0) {
      this.refs.pickerContent.style.left = targetRect.left - bodyRect.left + 'px';
    } else {
      this.refs.pickerContent.style.left = targetRect.right - bodyRect.left - pickerRect.width + 'px';
    }
  }
  ...
  handleBodyChange = (date, showType, typeChanged) => {
    const {showType: currentShowType} = this.state;
    const {type} = this.props;
    if (currentShowType === this.getDefaultShowType()) {
      if (type.timeType && type.timeType.slice(-1) !== typeChanged) {
  ...
  render() {
    ...
    const {
      handleClear, visible, type, handleCancel, limit, clearable
    } = this.props;
